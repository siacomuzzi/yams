  ...
  Yams.prototype.set = function(sid, session, callback) {
    ...
    if (session && session.cookie) {
      if (session.cookie._expires) {
        ...
        // So we set the expiration with `options.expires`
        // or `two-weeks` (default value, common practice
        // in the industry, e.g Django) from now.
        var today = new Date(),
        twoWeeks = 1000 * 60 * 60 * 24 * 14;
        var expires = typeof this.options.expires === 'number' ? this.options.expires : twoWeeks;
        s.expires = new Date(today.getTime() + expires);
      }
    }
